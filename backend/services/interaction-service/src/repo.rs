...
pub async fn insert_comment(
    ...
    if let Some(p) = parent_comment_id {
        let parent_of_parent: Option<Option<Uuid>> =
            sqlx::query_scalar("SELECT parent_comment_id FROM comments WHERE id=$1 AND post_id=$2")
                .bind(p)
                .bind(post_id)
...
pub async fn my_interactions(
    ...
    Ok(MyInteractions {
        liked: row.0.unwrap_or(false),
        saved: row.1.unwrap_or(false),
...
pub async fn batch_my_interactions(
    db: &sqlx::PgPool,
    user_id: Uuid,
    post_ids: &[Uuid],
    ...
    let rows: Vec<(Uuid, bool, bool, bool, bool, bool)> = sqlx::query_as(
        "SELECT
           ...
           COALESCE(bool_or(i.type = 'save'::interaction_type), false) AS saved,
           COALESCE(bool_or(i.type = 'share'::interaction_type), false) AS shared,
           COALESCE(bool_or(i.type = 'hide'::interaction_type), false) AS hidden,
           EXISTS (

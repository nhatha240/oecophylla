...
export const mockFeed: Post[] = Array.from({ length: 8 }).map((_, i) => ({
  ...
  view_count: 100 + i, like_count: 12 + i, comment_count: i, save_count: 3 + i, share_count: 1,
  created_at: new Date(Date.now() - i * 3.6e6).toISOString(), updated_at: new Date().toISOString(),
}));

<script lang="ts">
  ...
  $: displayTopic = data.topic.charAt(0).toUpperCase() + data.topic.slice(1);
  ...
  async function loadMore(): Promise<void> {
    ...
    error = null;
    try {
      const next = await getPostsByTopic(fetch, data.topic, cursor);
...
<svelte:head>
  <title>{displayTopic} — Oecophylla</title>
</svelte:head>
...
<div class="feed-grid">
  <main class="feed-main">
    ...
    <div style="display: flex; gap: 8px; margin-bottom: 4px; align-items: center;">
      <span class="chip active">{displayTopic}</span>
      <span class="t-meta">{items.length} bài viết</span>
    </div>
    <p class="t-meta" style="margin-bottom: 16px;">Chủ đề · Được phân loại tự động</p>

    {#if items.length === 0}
      <div class="card card-pad" style="text-align: center; padding: 48px 24px;">
        <h2 class="serif" style="font-size: 24px; margin: 0 0 8px;">Chưa có bài viết với chủ đề {displayTopic}</h2>
        <p class="muted" style="margin: 0;">Hãy quay lại sau hoặc thử chủ đề khác.</p>
      </div>
    {:else}
